<template>
  <div id="detail" class="slide_enter_active">
    <Header title="影片详情">
    ...
    <div v-else id="content" class="contentDetail">
      ...
      <div class="detail_list">
        <div
          ...
          :style="{ 'background-image' : 'url('+ detailMovie.img.replace(/w\.h/,'130.228') +')' }"
        ></div>
        <div class="detail_list_filter"></div>
        <div class="detail_list_content">
          <div class="detail_list_img">
            <img :src="detailMovie.img | setWH('158.208')" alt />
          </div>
          <div class="detail_list_info">
      ...
      <div class="detail_player swiper-container" ref="detail_player">
        <ul class="swiper-wrapper">
          <li v-for="(item,index) in detailMovie.photos" :key="index" class="swiper-slide">
            <div>
              <img :src="item | setWH('200.110')" alt />
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  mounted() {
    ...
    this.axios.get("/detail?movieId=" + this.movieId).then(res => {
      // console.log(res);
      var msg = res.status;
      // console.log(msg);
      if (msg === 200) {
