		...
		methods:{
			updateCity(){//更新市数据
				...
				this.arr.forEach( function(item, index) {
					if (item.name == _this.prov) {
						_this.cityArr = item.sub
					}
				});
				_this.city = _this.cityArr[1].name
			},
			updateDistrict(){//更新区数据
				var _this = this
				this.cityArr.forEach( function(item, index) {
					if (item.name == _this.city) {
						_this.districArr = item.sub
					}
				});
				 _this.districArr && _this.districArr.length>0 ? _this.district = _this.districArr[1].name :_this.district= '';
			},
		},
		mounted:function(){
			this.updateCity()
