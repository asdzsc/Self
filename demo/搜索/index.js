    // 利用对象收编变量
    // 好处：在团队开发中，要减少变量相互覆盖的问题，用一个对象收编变量，可以减少变量覆盖的风险，
    //       一旦变量被覆盖，所有的功能都会失效，此时，这种现象也是很明显的，错误排查起来会更快速
    ...
    var search = {
        ...
        render() { // 渲染函数
            this.renderRes(); // 执行 渲染搜索列表 函数
            this.renderSearchList(); // 执行 渲染搜索列表的显示与隐藏 函数
        },
        renderRes() { // 渲染搜索列表 函数
        ...
        renderSearchList() { // 渲染搜索列表的显示与隐藏 函数
            var length = this.searchResList.length;
            var inputFocus = this.inputFocus;
            if (length && inputFocus) {
                // 搜索列表长度存在 并且 输入框为聚焦状态
                this.searchListDom.style.display = 'block';
            } else {
                // 输入框为失焦状态
                this.searchListDom.style.display = 'none';
            }
        },
    }